...
class firebaseHelper:ObservableObject {
   ...
   // static var statusTanaman: StatusTanaman = StatusTanaman(kelembabanTanah: 0, kelembabanUdara: 0, suhu: 0)
    @Published var statusTanaman: StatusTanaman = StatusTanaman(kelembabanTanah: 0, kelembabanUdara: 0, suhu: 0, sinarMatahari: 0)
    func getDataFromFireBase(){
        ref.child("device1").observe(DataEventType.value) { DataSnapshot in
            ...
            guard let sinarMatahari = DataSnapshot.childSnapshot(forPath: "lux").value as? Double else {return}
            ...
            self.statusTanaman.sinarMatahari = sinarMatahari
        }
    }
}
